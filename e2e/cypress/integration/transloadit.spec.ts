describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ ImageTestVarLang }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(ImageTestVarLang.getPlugin('Transloadit').activeAssemblies).every(
            (a: any) => a.pollInterval,
          ),
        ).to.equal(true)

        ImageTestVarLang.cancelAll()

        cy.wait(['@delete']).then(() => {
          // eslint-disable-next-line
          // @ts-ignore fix me
          expect(
            Object.values(ImageTestVarLang.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ ImageTestVarLang }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      ImageTestVarLang.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ ImageTestVarLang }) => {
      ...
      const { files } = ImageTestVarLang.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      ImageTestVarLang.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ ImageTestVarLang }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(ImageTestVarLang.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it.skip('should create assembly if there is still one file to upload', () => {
    ...
    cy.window().then(({ ImageTestVarLang }) => {
      ...
      ImageTestVarLang.removeFile(fileID)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ ImageTestVarLang }) => {
      const { files } = ImageTestVarLang.getState()
      ImageTestVarLang.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        ImageTestVarLangserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'ImageTestVarLang-core:3.2.0,ImageTestVarLang-transloadit:3.1.3,ImageTestVarLang-tus:3.1.0,ImageTestVarLang-dropbox:3.1.1,ImageTestVarLang-box:2.1.1,ImageTestVarLang-facebook:3.1.1,ImageTestVarLang-google-drive:3.1.1,ImageTestVarLang-instagram:3.1.1,ImageTestVarLang-onedrive:3.1.1,ImageTestVarLang-zoom:2.1.1,ImageTestVarLang-url:3.3.1',
    ...
    cy.get('.ImageTestVarLang-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.ImageTestVarLang-StatusBar-actions > .ImageTestVarLang-c-btn').click()
    cy.wait([
